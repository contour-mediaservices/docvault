...
if ($action === "create" && !empty($_POST['name'])) {
    $pdo->prepare("INSERT IGNORE INTO project_tags (name) VALUES (?)")
...
if ($action === "update" && !empty($_POST['id']) && isset($_POST['name'])) {
    $pdo->prepare("UPDATE project_tags SET name=? WHERE id=?")
...
if ($action === "delete" && !empty($_GET['id'])) {
    ...
    if ($check->fetchColumn() == 0) {
        $pdo->prepare("DELETE FROM project_tags WHERE id=?")->execute([$id]);
    }
...
/* =====================================================
   DATA
===================================================== */
$totalRows = (int)$pdo->query("SELECT COUNT(*) FROM project_tags")->fetchColumn();
$pagination = dvPagination($totalRows);

$tags = $pdo->query("
SELECT t.*, COUNT(m.project_id) AS usage_count
FROM project_tags t
LEFT JOIN project_tag_assignments m ON t.id = m.tag_id
GROUP BY t.id
ORDER BY t.name
