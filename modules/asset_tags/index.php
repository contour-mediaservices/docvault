...
if ($action === "create" && !empty($_POST['name'])) {
    ...
    if ($name !== "") {
        $pdo->prepare("INSERT IGNORE INTO asset_tags (name) VALUES (?)")->execute([$name]);
    }
...
if ($action === "update" && !empty($_POST['id']) && isset($_POST['name'])) {
    $pdo->prepare("UPDATE asset_tags SET name=? WHERE id=?")
...
if ($action === "delete" && !empty($_GET['id'])) {
    ...
    $cnt = $pdo->prepare("SELECT COUNT(*) FROM asset_tag_assignments WHERE tag_id=?");
    $cnt->execute([$id]);

    if ($cnt->fetchColumn() == 0) {
        $pdo->prepare("DELETE FROM asset_tags WHERE id=?")->execute([$id]);
    }
...
/* =====================================================
   DATA
===================================================== */
$totalRows = (int)$pdo->query("SELECT COUNT(*) FROM asset_tags")->fetchColumn();
$pagination = dvPagination($totalRows);

$tags = $pdo->query("
SELECT t.*, COUNT(m.asset_id) AS usage_count
FROM asset_tags t
LEFT JOIN asset_tag_assignments m ON t.id = m.tag_id
GROUP BY t.id
ORDER BY t.name
